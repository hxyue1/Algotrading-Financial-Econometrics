...
Definitely in improvement but still problems in the left tail, let's trying using allowing for skewness in the student's t
...
```{r}
sim = ugarchsim(vaf_gjr_garch_fit,n.sim=2500)
sim_std_resids = sim@simulation[["residSim"]]/sim@simulation[["sigmaSim"]]
skewhypFit(sim_std_resids)
```

Estimated parameters from simulated model seem pretty consistent with empirical values. Let's go with the skewed Student's-t distribution for the errors.

Ok, in that case what does the shape and skew parameter correspond to in the estimation output?
...
QQPlot looks pretty good, but Shapiro-Wilks test still strongly rejects normality. Let's try student's t distribution.
...
Student's t distribution seems to be an even better fit, judging by the QQPlot. One extreme value though in the left tail.
...
qqt(vgs_std_resids,df=5.849125)
qqline(vgs_std_resids, col = "steelblue", lwd = 2)
rownames(vgs_std_resids) <- c()
shapiro.test(vgs_std_resids$vgs_std_resids)
```

Both Normal and Student's-t distribution seem to be quite poor fits because of the very heavy left tail. Massive price falls that the model cannot account for. 
